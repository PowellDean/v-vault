...
pub fn (c Client) delete_secret_v1(mountpoint string, secret string) API_error {
	...
	header.add_custom('X-Vault-Token', c.token) or { panic('Cannot create request header') }
	...
	match response.status() {
		...
		.forbidden {
			...
			return API_error{
				errors: ['Unknown error in delete_secret_v1']
			}
		}
	}
}

pub fn (c Client) delete_secret_v2(mountpoint string, secret string) API_error {
	mut header := http.new_header()
	header.add_custom('X-Vault-Token', c.token) or { panic('Cannot create request header') }
...
pub fn (c Client) delete_secret_v2_versions(mountpoint string, secret string, versions []int) API_error {
	mut header := http.new_header()
	header.add_custom('X-Vault-Token', c.token) or { panic('Cannot create request header') }
	...
	response := req.do() or { return API_error{
		errors: [err.msg]
	} }
	...
	match response.status() {
		.no_content {
			...
			return API_error{
				errors: ['Unknown error occurred delete_secret_v2_versions']
			}
		}
	}
}

pub fn (c Client) destroy_secret_v2_versions(mountpoint string, secret string, versions []int) API_error {
	mut header := http.new_header()
	header.add_custom('X-Vault-Token', c.token) or { panic('Cannot create request header') }
	...
	match response.status() {
		...
		.not_found {
			return API_error{
				errors: ['$mountpoint not found']
			}
		}
		else {
			return API_error{
				errors: ['Unknown error in destroy_secret_v2_versions']
			}
		}
	}
}
...
pub fn (c Client) get_secret_v1(mountpoint string, key string) (Secret_v1, API_error) {
	mut header := http.new_header()
	header.add_custom('X-Vault-Token', c.token) or { panic('Cannot create request header') }
...
pub fn (c Client) get_secret_v2(mountpoint string, key string, ver int) (Secret_v2, API_error) {
	mut header := http.new_header()
	header.add_custom('X-Vault-Token', c.token) or { panic('Cannot create request header') }
...
pub fn (c Client) list_secrets(mountpoint string) (Key_list, API_error) {
	...
	match response.status() {
		...
		.not_found {
			failure = API_error{
				errors: ['Mountpoint $mountpoint not found']
			}
			return res, failure
		}
		else {
...
fn new_client_via_token(address string, token string) Client {
	...
	match true {
		...
		host_elements.len == 2 {
			...
			panic('Error parsing given Vault address')
		}
	}
...
fn new_client_via_login(address string, uname string, pswd string) Client {
	...
	response := req.do() or { panic(err) }

	match response.status() {
		.ok {
			wanted_response := json.decode(User_login_response, response.text) or { panic(err) }
...
pub fn (c Client) is_initialized() bool {
	...
	response := http.get(url) or { panic(err) }
...
pub fn (c Client) is_sealed() bool {
	...
	match response.status() {
		.ok {
			status := json.decode(Status, response.text) or { panic(err) }
...
pub fn (c Client) put_secret_v1(mountpoint string, key string, new_key string, new_value string) API_error {
	mut header := http.new_header()
	header.add_custom('X-Vault-Token', c.token) or { panic('Cannot create request header') }
	...
	match response.status() {
		...
		.bad_request {
			...
			return API_error{
				errors: ['Unknown error in put_secret_v1']
			}
		}
	}
}

pub fn (c Client) put_secret_v2(mountpoint string, key string, new_key string, new_value string) API_error {
	mut header := http.new_header()
	header.add_custom('X-Vault-Token', c.token) or { panic('Cannot create request header') }
	...
	match response.status() {
		...
		.bad_request {
			...
			return API_error{
				errors: ['Unknown error in put_secret_v2']
			}
		}
	}
}
...
pub fn (c Client) read_policy(name string) {
	...
	match response.status() {
		.ok {
			wanted_response = json.decode(Policy, response.text) or { panic(err) }
...
pub fn (c Client) token_lookup(tkn string) (Token_data, API_error) {
	mut header := http.new_header()
	header.add_custom('X-Vault-Token', tkn) or { panic('Cannot create request header') }
	header.add_custom('accept', '*/*') or { panic('Cannot create request header') }
...
pub fn (c Client) undelete_secret_v2_versions(mountpoint string, secret string, versions []int) bool {
	mut header := http.new_header()
	header.add_custom('X-Vault-Token', c.token) or { panic('Cannot create request header') }
